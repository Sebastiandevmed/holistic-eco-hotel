...
function TheCabin() {
  ...
  return (
    <section id="La cabaña" style={{ background: 'transparent', padding: 'clamp(64px, 10vw, 128px) 0 96px', position: 'relative', overflow: 'hidden' }}>
      <ParallaxBackdrop image="../../assets/RoomOut.jpg" />
      ...
      <div className="cabin-grid" style={{ position: 'relative', zIndex: 1, maxWidth: 1280, margin: '0 auto', padding: '28px 24px 0', display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 32, alignItems: 'start' }}>
        ...
        <aside className="cabin-card" style={{ position: 'sticky', top: 96, background: 'var(--glass-bg)', WebkitBackdropFilter: 'var(--glass-blur)', backdropFilter: 'var(--glass-blur)', border: '1px solid var(--glass-border)', borderRadius: 'var(--radius-lg)', padding: '30px 28px', boxShadow: 'var(--shadow-2)' }}>
          ...
          <ul style={{ listStyle: 'none', margin: '0 0 26px', padding: 0, display: 'grid', gap: 12 }}>
            {amenities.map((a) => (
              <li key={a} style={{ display: 'flex', gap: 10, alignItems: 'center', fontFamily: 'var(--font-body)', fontSize: 14, color: 'var(--fg)' }}>
                <span style={{ color: 'var(--brand)', display: 'inline-flex', flex: '0 0 auto' }}><IconLucide d="M20 6L9 17l-5-5" size={18} /></span>
                {a}
              </li>
            ))}
          </ul>
